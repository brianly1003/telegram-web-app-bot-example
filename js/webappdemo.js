var DemoApp = {
  ...
  checkInitData: function () {
    if (
      ...
      DemoApp.apiRequest("checkInitData", {}, function (result) {
        if (result.ok) {
          $("#webview_data_status")
            .text("Hash is correct (async)")
            .addClass("ok");
        } else {
          $("#webview_data_status")
            .text(result.error + " (async)")
            .addClass("err");
        }
      });
    }
  ...
  requestVideo: function (el) {
    if (navigator.mediaDevices) {
      ...
      $(el)
        .next("span")
        .text("Media devices is not supported in this browser.")
        .attr("class", "err");
    }
    return false;
  },
  ...
  testClipboard: function (el) {
    Telegram.WebApp.readTextFromClipboard(function (clipText) {
      if (clipText === null) {
        $(el)
          .next("span")
          .text("Clipboard text unavailable.")
          .attr("class", "err");
      } else {
        $(el)
          .next("span")
          .text("(Read from clipboard: «" + clipText + "»)")
          .attr("class", "ok");
      }
    });
    return false;
  ...
  requestWriteAccess: function (el) {
    Telegram.WebApp.requestWriteAccess(function (allowed) {
      if (allowed) {
        ...
        $(el)
          .next("span")
          .text("(User declined this request)")
          .attr("class", "err");
      }
    });
  },
    ...
    Telegram.WebApp.invokeCustomMethod(
      ...
      function (err, time) {
        if (err) {
          ...
          $(el)
            .next("span")
            .text("(" + new Date(time * 1000).toString() + ")")
            .attr("class", "ok");
        }
      }
    );
  ...
  biometricInit: function (el) {
    ...
    if (!DemoApp.biometricInited) {
      ...
      Telegram.WebApp.onEvent("biometricManagerUpdated", function () {
        ...
        $("#bm_access_requested").text(
          biometricManager.isAccessRequested ? "true" : "false"
        );
        $("#bm_access_granted").text(
